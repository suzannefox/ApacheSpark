...
# This file - Usage
# ===========================================================================
# General Usage -> spark-submit CW-Part1.py ARG2 ARG3
#
# Arg 2 is MODE, you can run the whole TF.IDF creation in one go, or in stages. Each stage creates a .CSV which 
#               the next stage uses to know what to do
#
#         MODES = ['ALL','TF','IDF','TF.IDF','CALC.IDF','NAIVE-BAYES','DECISION-TREES','LOGISTIC-REGRESSION']
#
# Arg 3 is stop_after, you can chose how many .txt files to process so easy to adjust the scope of the run
#                     this arg doesn't apply to TF.IDF runs because you have to create the TF.IDFs 
#                     for whatever files went into the IDF or it doesn't make any sense.
#                     default value is 20
#
#
# Example -
#
# spark-submit CW-Part1.py TF 80
# spark-submit CW-Part1.py IDF 80
# spark-submit CW-Part1.py TF.IDF
#
# OR
#
# spark-submit CW-Part1.py ALL 80
#
# THEN
#
# spark-submit CW-Part1.py 'NAIVE-BAYES'
# spark-submit CW-Part1.py 'DECISION-TREES'
# spark-submit CW-Part1.py 'LOGISTIC-REGRESSION'
#
# =====================================================================================
#
...
#
# Stage 1 - TF Creation
# - iterate through .txt files on disk up to stop_after files are successfully processed
...
# - write a record to sparkrun_tf.csv with file_name, FOLD, ID, Subjects
#
# Stage 2 - IDF Creation
# - iterate through sparkrun_tf.csv created at Stage 1 for stop_after number of files
...
# - write record (with id/fold/subjects) to sparkrun_idf.csv. if I want to do
#   an analysis on a subset then I have the subjects just for that subset of files
# - create IDF from TFs specified. 
#
# Stage 3 - TF.IDF
# - iterate through sparkrun_idf.csv created at Stage 2. All files from Stage 2 have
#   to be included or else the IDF makes no sense, so stop_after is not relevant
# - Calculate TF.IDFs
# - write to Fold directory specified in the sparkrun_idf.csv
#
# =====================================================================================
...
if __name__ == "__main__":    
    # ==================================================================
    # Arg 2 is MODE
    # Arg 3 is stop_after (optional , default is 20)
    # ==================================================================
    if len(sys.argv) < 2:
        print >> sys.stderr,"Supply arguments - MODE <optional> number_to_process"
        exit(-1)

    # I can run this from home (PC or mac) or lewes, started with the PC at home 
    # which blew up, but it was a good things as 
...
#    I experimented with using log or not for the IDF calculation
#    if MODE == 'CALC.IDF':
#        # Finished processing
#        print "Mapping ..."
#        N = 50.0
